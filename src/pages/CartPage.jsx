              ...
              <List>
                {cartItems.map((item) => (
                  <Box key={item.id} sx={{ mb: 2 }}>
                    ...
                    >
                      <ListItemAvatar>
                        <Avatar alt={item.title} src={item.image} variant="square" sx={{ width: 60, height: 60, objectFit: 'contain', borderRadius: 2, background: '#f5f5f5' }} />
                      </ListItemAvatar>
                      <ListItemText
                        primary={<Typography sx={{ fontWeight: 600, fontSize: '1rem', color: 'text.primary' }}>{item.title}</Typography>}
                        secondary={<Typography sx={{ color: 'text.secondary', fontSize: '0.9rem' }}>{`$${item.price} · ${item.category}`}</Typography>}
                        sx={{ ml: 2 }}
                      />
                      <Box sx={{ display: 'flex', alignItems: 'center', ml: 2, background: '#f9fbe7', borderRadius: 2, px: 1 }}>
                      ...
                      <Typography variant="body1" sx={{ ml: 2, mr:3, fontWeight: 700, color: 'primary.main', minWidth: 70, textAlign: 'right' }}>
            ...
            >
              <CardContent>
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
                  ...
                  <Typography variant="h6" sx={{ fontWeight: 700 }}>${calculateTotal().toFixed(2)}</Typography>
                </Box>
                <Button
                  variant="contained"
                  fullWidth
